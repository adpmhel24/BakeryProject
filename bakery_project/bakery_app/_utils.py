...
class Check():
    ...
    def itemgroup_exist(self):
        if ItemGroup.query.filter_by(code=self.group_code).first():
            return True
        return False
    ...
    def branch_exist(self):
        if Branch.query.filter_by(code=self.code).first():
            return True
        return False
...
class ResponseMessage:
    ...
    def __init__(self, success, message=None, data=None, token=None):
